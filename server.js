...
app.post('/api/next-question', async (req, res) => {
  try {
    ...
    if (answers.length > 0) {
      messages.push({
        role: 'user',
        content: `Previous answers: ${JSON.stringify(answers)}. Based on these ${answers.length} answers, generate your next question. Remember to switch to a DIFFERENT category than your last question. Return JSON response.`
      });
    } else {
      messages.push({
        role: 'user',
        content: 'Start the gift questionnaire. Ask your first question to understand who this gift is for. Return JSON response.'
      });
    }
