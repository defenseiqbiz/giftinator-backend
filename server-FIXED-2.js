    ...
    if (answers.length > 0) {
      messages.push({
        ...
        content: `Previous conversation:\n${answers.map((a, i) => 
          `Q${i + 1}: ${a.question}\nA${i + 1}: ${a.answer}`
        ).join('\n\n')}\n\nBased on these ${answers.length} answers, what's your next question? Remember to switch to a DIFFERENT category than your last question. Return your response in JSON format.`
      });
    } else {
      messages.push({
        role: 'user',
        content: 'Start the gift questionnaire. Ask your first question to understand who this gift is for. Return your response in JSON format.'
      });
    }
