...
// Helper: Add affiliate tag to Amazon URL
function addAffiliateTag(url, tag) {
  if (!url) return null;
  const hasQuery = url.includes('?');
  const delimiter = hasQuery ? '&' : '?';
  return `${url}${delimiter}tag=${encodeURIComponent(tag)}`;
}
...
${answers.length === 0 ? 'START WITH Q1: Ask for the giftee\'s NAME. DO NOT provide options. Set options to empty array []. The user will type their answer. Just ask: "What\'s their name?" Return JSON with empty options array.' : ''}
${answers.length === 1 ? 'Q2: Ask about their RELATIONSHIP to the giftee (partner, friend, parent, etc). Provide 4 clear options plus "None of these - let me explain". Return JSON.' : ''}
${answers.length === 2 ? 'Q3: Ask for their AGE RANGE. Provide options: Under 18, 18-25, 26-35, 36-50, 51-65, 65+, plus "None of these - let me explain". Return JSON.' : ''}

CRITICAL: Review the questions you've already asked above. DO NOT repeat or ask similar questions. Ask something NEW that builds on what you already know.
